...
export function AdultContentControls() {
  ...
  const handleAdultToggle = () => {
    if (adultMode) {
      savePreferences({ adultMode: false }, true)
      return
    }
    if (adultUnlocked) {
      savePreferences({ adultMode: true }, true)
      return
    }
    setUnlockError('')
    setUnlockOpen(true)
  ...
  const handleUnlockSubmit = async (key: string) => {
    setUnlockError('')
    try {
      await unlockAndEnableAdult(key)
      setUnlockOpen(false)
    } catch (e) {
  ...
  return (
    <>
        ...
        {open && adultMode && (
          <div className="absolute top-full right-0 mt-1.5 z-[60] w-[min(92vw,22rem)] max-h-[min(70vh,32rem)] overflow-y-auto rounded-lg glass-panel-opaque shadow-2xl p-3 space-y-3">
            <div className="private-mode-banner rounded-lg border px-3 py-2 space-y-1 bg-game-surface/90">
              ...
              {adultUnlockKeyMasked && (
                <p className="text-[10px] text-game-dim">密钥 {adultUnlockKeyMasked}</p>
              )}
            </div>
      ...
      <AdultUnlockDialog
        ...
        saving={saving}
        error={unlockError}
        maskedKey={adultUnlockKeyMasked}
